...
namespace Ravl2::Video
{
      ...
      , m_metadata(metadata)
  {
    ...
    {
      MediaTime streamDuration = std::visit([](const auto&props) -> MediaTime { return props.duration; },
                                            stream.mProperties
      );
      ...
      case SeekFlags::Precise:
        {
          auto it = std::find_if(m_frames->begin(),
                                 m_frames->end(),
                                 [timestamp](const auto&f) { return f->timestamp() == timestamp; }
          );
          if (it == m_frames->end())
          {
            // Choose closest
            it = std::min_element(m_frames->begin(),
                                  m_frames->end(),
                                  [timestamp](const auto&a, const auto&b)
                                  {
                                    auto da = (a->timestamp() - timestamp).count();
                                    if (da < 0) da = -da;
                                    auto db = (b->timestamp() - timestamp).count();
                                    if (db < 0) db = -db;
                                    return da < db;
                                  }
            );
      ...
      case SeekFlags::Previous:
        {
          ...
          {
            if (m_frames->at(static_cast<std::size_t>(i))->timestamp() <= timestamp)
            {
              newPos = i;
              break;
            }
      ...
      case SeekFlags::Next:
        {
          ...
          {
            if (m_frames->at(i)->timestamp() >= timestamp)
            {
              newPos = static_cast<std::ptrdiff_t>(i);
              break;
            }
          }
          break;
  ...
  {
    if (!m_frames) return VideoResult<void>(VideoErrorCode::InvalidOperation);
    if (index < 0 || static_cast<std::size_t>(index) >= m_frames->size()) return VideoResult<void>(
  ...
  VideoResult<void> MemoryStreamIterator::reset()
  {
    if (!m_frames) return VideoResult<void>(VideoErrorCode::InvalidOperation);
    m_currentPosition = 0;
    if (!m_frames->empty()) setCurrentFrame(m_frames->at(0));
    else setCurrentFrame(nullptr);
    return VideoResult<void>();
  }
