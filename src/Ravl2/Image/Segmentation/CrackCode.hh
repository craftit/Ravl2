...
namespace Ravl2
{
  ...
  inline constexpr Index<2> crackStep(const Index<2> &pixel, CrackCodeT crackCode)
  {
    return pixel + CrackCode::offset[unsigned(crackCode)];
  }

  //! Direction in the form of an offset for a crack code.
  inline constexpr Index<2> crackDirection(CrackCodeT crackCode)
  {
...
{
  template <>
  struct formatter<Ravl2::CrackCodeT> : ostream_formatter {
  };
  template <>
  struct formatter<Ravl2::CrackCode> : ostream_formatter {
  };
}// namespace fmt
