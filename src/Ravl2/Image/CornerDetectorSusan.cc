    ...
    for(int i = rowMin; i <= rowMax; i++)
      for(int j = colMin; j <= colMax; j++) {
        ...
        if(yy < sq) {
          ...
          sq = std::abs(x) / x;
          sq = *(cp - img[(i + FTOI(divide))][j + sq]) + *(cp - img[(i + FTOI(2 * divide))][j + 2 * sq]) + *(cp - img[(i + FTOI(3 * divide))][j + 3 * sq]);
        } else if(xx < sq) {
          float divide = float(x) / float(std::abs(y));
          sq = std::abs(y) / y;
          sq = *(cp - img[(i + sq)][j + FTOI(divide)]) + *(cp - img[(i + 2 * sq)][j + FTOI(2 * divide)]) + *(cp - img[(i + 3 * sq)][j + FTOI(3 * divide)]);
        }
        if(sq <= 290)
          continue;
