...
namespace Ravl2
{
  ...
  class Affine2f : public Affine<float, 2>
  {
  public:
    //! Construct identity transformation.
    inline Affine2f() = default;

    //! Copy constructor.
    inline explicit Affine2f(const Affine<float, 2> &oth)
        : Affine<float, 2>(oth)
    {}

    //! Constructor from scaling/rotation matrix and translation vector.
    inline Affine2f(const Matrix<float, 2, 2> &SR, const Vector<float, 2> &T)
        : Affine<float, 2>(SR, T)
    {}

    //
    //    inline Affine2f(const Vector2dC &Scale, RealT Angle, const Vector2dC &Trans);
    ...
    //
    //    inline Affine2f(const Vector2dC &Scale, RealT ScaleAngle, RealT Angle, const Vector2dC &Trans);
    ...
    //
    //    static Affine2f Compose(Point2dC translation,Vector2dC scaling,RealT skew,RealT rotation);
  ...
  //
  //  Affine2f FitAffine(const SArray1dC<Point2dC> &orig,const SArray1dC<Point2dC> &newPos,RealT &residual);
  ...
  //
  //  Affine2f FitAffine(const SArray1dC<Point2dC> &orig,const SArray1dC<Point2dC> &newPos);
  //  //: Fit an affine transform to 2 arrays of corresponding points
  //  // A "least sum of squares" fitter is used.  The result transforms the points in "orig" to those in "newPos".
  //
  //
  //
  //  Affine2f FitAffine(const SArray1dC<Point2dC> &org,const SArray1dC<Point2dC> &newPos, const SArray1dC<RealT> &weights, RealT& residual);
  //  //: Fit an affine transform to 2 arrays of corresponding points
  //  // A "weighted least sum of squares" fitter is used.  The result transforms the points in "orig" to those in "newPos".
  //
  //
  //  Affine2f FitAffine(const SArray1dC<Point2dC> &orig,const SArray1dC<Point2dC> &newPos, const SArray1dC<RealT> &weights);
  //  //: Fit an affine transform to 2 arrays of corresponding points
  //  // A "least sum of squares" fitter is used.  The result transforms the points in "orig" to those in "newPos".
  //
  //
  //  Affine2f FitAffine(const DListC<Point2dC> &orig,const DListC<Point2dC> &newPos,RealT &residual);
  //  //: Fit an affine transform to 2 lists of corresponding points
  //  // A "least sum of squares" fitter is used.  The result transforms the points in "orig" to those in "newPos". The residual from the fit is returned as "residual".
  //
  //
  //  Affine2f FitAffine(const DListC<Point2dC> &orig,const DListC<Point2dC> &newPos);
  //  //: Fit an affine transform to 2 lists of corresponding points
  //  // A "least sum of squares" fitter is used.  The result transforms the points in "orig" to those in "newPos".
  //
  //  Affine2f FitAffine(const SArray1dC<PairC<Point2dC> > &matchPairs,RealT &residual);
  ...
  //
  //  Affine2f FitAffine(const DArray1dC<PairC<Point2dC> > &matchPairs,RealT &residual);
  //  //: Fit an affine transform to a list of corresponding points
  //  // A "least sum of squares" fitter is used.  The result transforms the points in element 1 of the point pairs to those in element 2 of the point pairs. The residual from the fit is returned as "residual".
  //
  ...
  //
  //  inline
  //  Affine2f::Affine2f(const Vector2dC &Sc, RealT Angle, const Vector2dC &Trans)
  ...
  //
  //  inline
  //  Affine2f::Affine2f(const Vector2dC &Sc, RealT ScaleAngle, RealT Angle, const Vector2dC &Trans)
  //    : FAffineC<2>(Matrix2dC(1,0,0,1),Trans)
  //  {
  //    Rotate(-ScaleAngle);
  //    Scale(Sc);
