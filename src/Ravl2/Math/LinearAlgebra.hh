...
namespace Ravl2
{
  ...
  [[nodiscard]] inline std::optional<Matrix<RealT, 2, 2>> inverse(const Matrix<RealT, 2, 2> &data)
  {
    ...
    if(isNearZero(det))
      return std::nullopt;
    return Matrix<RealT, 2, 2> {{{data(1, 1) / det, -data(0, 1) / det},
  ...
  //! Instantiate the inverse function for 2x2 and 3x3 matrices
  extern template std::optional<Matrix<float, 2, 2>> inverse(const Matrix<float, 2, 2> &data);
  extern template std::optional<Matrix<float, 3, 3>> inverse(const Matrix<float, 3, 3> &data);
