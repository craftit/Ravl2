...
#include "Ravl2/Display/AddSeriesData.hh"
#include "Ravl2/Display/ClearPlot.hh"
  ...
  std::shared_ptr<IRenderCommand> makeCmdFromRGB32FArray(const Array<PixelRGB32F,2> &img)
{
  ...
  return cmd;
    ...
    // Converter: Array<float,1> -> shared_ptr<IRenderCommand> (AddSeriesData)
    std::shared_ptr<IRenderCommand> makeCmdFromF32Array1D(const Array<float, 1> &arr)
    {
      // Default series name will be "data", channel and mode set by sink from URL
      auto cmd = std::make_shared<AddSeriesData>(std::string{}, "data");
      int size = arr.range()[0].size();
      cmd->y.reserve(static_cast<size_t>(size));
      ...
      // X values will be auto-generated by AddSeriesData command
      return cmd;
    }

    // Converter: std::vector<float> -> shared_ptr<IRenderCommand> (AddSeriesData)
    std::shared_ptr<IRenderCommand> makeCmdFromVectorFloat(const std::vector<float> &vec)
    {
      auto cmd = std::make_shared<AddSeriesData>(std::string{}, "data");
      cmd->y.reserve(vec.size());
      for(float val : vec) {
        if(std::isfinite(val)) {
          cmd->y.push_back(val);
        }
    ...
    inline bool parseColorRgba(std::string val, uint32_t &out)
    {
      ...
      if(!val.empty() && val[0] == '#') {
        if(val.size() == 9) {
          ...
          if(r >= 0 && g >= 0 && b >= 0 && a >= 0) {
            out = packColorRGBA255(uint8_t(r), uint8_t(g), uint8_t(b), uint8_t(a));
            return true;
          }
        }
        return false;
            ...
            if(isImageCmd) {
              ...
              if(auto mv = getControlValue(parsed2->controls, ":Mode=")) {
                if(*mv == "Replace") poly->mode = AddPolylineOverlay2D::Mode::Replace;
                  ...
                  poly->mode = AddPolylineOverlay2D::Mode::Append;
              }
              // Closed
              ...
              if(auto wv = getControlValue(parsed2->controls, ":Width=")) {
                try {
                  poly->widthPx = std::max(0.1f, std::stof(*wv));
                } catch(...) {}
              }
              // Color
              if(auto colv = getControlValue(parsed2->controls, ":Color=")) {
                uint32_t rgba {};
                if(parseColorRgba(*colv, rgba)) poly->rgba = rgba;
                else {
              ...
              DebugDisplay::enqueue(cmd);
            } else if(auto *plotCmd = dynamic_cast<AddSeriesData *>(cmd.get())) {
              // Handle plot series data with URL controls
              plotCmd->channel = parsed2->channel;

              // :Series=<name> - series identifier (default: "data")
              if(auto sv = getControlValue(parsed2->controls, ":Series=")) {
                plotCmd->seriesName = *sv;
              }

              // :Mode=Replace|Append|RingBuffer (default: Append)
              if(auto mv = getControlValue(parsed2->controls, ":Mode=")) {
                if(*mv == "Replace") plotCmd->mode = SeriesUpdateMode::Replace;
                else if(*mv == "RingBuffer") plotCmd->mode = SeriesUpdateMode::RingBuffer;
                else plotCmd->mode = SeriesUpdateMode::Append;
              }

              // :MaxPoints=<n> - ring buffer size limit
              if(auto mp = getControlValue(parsed2->controls, ":MaxPoints=")) {
                try {
                  size_t maxPts = std::max(size_t(10), static_cast<size_t>(std::stoul(*mp)));
                  ...
                  // For now, just log it.
                  SPDLOG_DEBUG("DebugDisplay: :MaxPoints={} specified (applied per-channel)", maxPts);
                } catch(...) {}
              }

              // :ClearPlot or :ClearSeries - clear before adding
