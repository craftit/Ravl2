...
namespace Ravl2
{
  ...
  constexpr RealT sign(RealT a, RealT b)
  ...
  [[nodiscard]] constexpr bool isPow2(IntT i)
  {
    IntT j = 1;
    while(j < i)
      j *= 2;
  ...
  template <class RealT>
   requires std::is_floating_point<RealT>::value
  constexpr bool isNearZero(RealT x, RealT tol = std::numeric_limits<RealT>::epsilon())
  {
  ...
  constexpr DataT sqr(const DataT &x)
  {
    return x * x;
  }
