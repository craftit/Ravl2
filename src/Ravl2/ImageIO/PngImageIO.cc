  ...
  namespace {
    ...
    [[maybe_unused]] bool g_regPngIn = inputFormatMap().add(std::make_shared<InputFormatCall>(
      ...
      [](const ProbeInputContext &ctx) -> std::optional<StreamInputPlan>
      {
        ...
        struct Candidate {
          enum class Kind { RGBA8, RGB8, Gray8, RGB16, RGBA16, Gray16 } kind;
          float score;
        };
    ...
    [[maybe_unused]] bool g_regPngOut = outputFormatMap().add(std::make_shared<OutputFormatCall>(
      ...
      [](const ProbeOutputContext &ctx) -> std::optional<StreamOutputPlan>
      {
        ...
        enum class Choice { None, RGBA8, RGB8, Gray8, RGB16, RGBA16, Gray16 } choice = Choice::None;
        ...
        try {
          ...
          if (ctx.m_formatHint.contains("pngFilter")) {
            ...
            if (f == "none") filter = PNG_FILTER_NONE; else if (f == "sub") filter = PNG_FILTER_SUB; else if (f == "up") filter = PNG_FILTER_UP; else if (f == "avg") filter = PNG_FILTER_AVG; else if (f == "paeth") filter = PNG_FILTER_PAETH; else filter = PNG_ALL_FILTERS;
          }
        ...
        if (choice == Choice::RGB8) {
          ...
          auto strm = std::make_unique<StreamOutputCall<ViaT>>([filename = ctx.m_filename, compression, interlace, filter](const ViaT &img, std::streampos pos) -> std::streampos {
            ...
            if (!pngPtr) { std::fclose(f); throw std::runtime_error("png_create_write_struct failed"); }
            png_infop infoPtr = png_create_info_struct(pngPtr);
            if (!infoPtr) { png_destroy_write_struct(&pngPtr, nullptr); std::fclose(f); throw std::runtime_error("png_create_info_struct failed"); }
            if (setjmp(png_jmpbuf(pngPtr))) { png_destroy_write_struct(&pngPtr, &infoPtr); std::fclose(f); throw std::runtime_error("libpng write error"); }
